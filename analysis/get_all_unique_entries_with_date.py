...
Script to get all unique entries in all playlists by date
...
if __name__ == '__main__':
    ...
    for playlist in os.listdir(rootdir):
        ...
        for file in os.listdir(rootdir + '/' + playlist):           
            ...
            df_temp = pd.read_csv(filepath)[index_cols]
            df_temp['date'] = re.sub('.csv', '', file)
            dfs_playlist.append(df_temp)
        ...
        df_concat_all = pd.concat(dfs_all).\
            drop_duplicates(subset=[index_cols[1], 'date'])
        dfs_all = [df_concat_all]
        
    assert len(dfs_all) == 1, 'dfs_all does not have length 1'
    # save to csv
    dfs_all[0].to_csv(
            rootdir + playlist_type + '_unique_' + 
            index_column + '.csv', index=False)
